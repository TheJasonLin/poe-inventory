...
object WeaponFactory {
  def create(rarity: String, base: String, name: Option[String], itemLevel: Int, identified: Boolean, quality: Int): Option[Weapon] = {
    if (base.contains("Bow")) {
      return Option(new Bow(rarity, base, name, itemLevel, identified, quality))
    }

    if (Item.matchesIdentifier(base, Axe.identifiers)) {
      if (Axe.oneHandedBases.contains(base)) {
        return Option(new Axe(rarity, base, name, itemLevel, identified, quality, true))
      } else {
        return Option(new Axe(rarity, base, name, itemLevel, identified, quality, false))
      }
    }

    if (Item.matchesIdentifier(base, Claw.identifiers)) {
      return Option(new Claw(rarity, base, name, itemLevel, identified, quality))
    }
    ...
    if (
      ...
      return Option(new Dagger(rarity, base, name, itemLevel, identified, quality))
    }
    ...
    if (Item.matchesIdentifier(base, Mace.identifiers)) {
      ...
      if (matchesOneHandedIdentifier || matchesOneHandedBase) {
        return Option(new Mace(rarity, base, name, itemLevel, identified, quality, true))
      } else {
        return Option(new Mace(rarity, base, name, itemLevel, identified, quality, false))
      }
    }

    if (Item.matchesIdentifier(base, Staff.identifiers)) {
      return Option(new Staff(rarity, base, name, itemLevel, identified, quality))
    }
    ...
    if (
      ...
      if (Item.matchesIdentifier(base, Sword.oneHandedIdentifiers) || Sword.oneHandedBases.contains(base)) {
        ...
        return Option(new Sword(rarity, base, name, itemLevel, identified, quality, true, isThrusting))
      } else {
        return Option(new Sword(rarity, base, name, itemLevel, identified, quality, false, false))
      }
    }

    if (Item.matchesIdentifier(base, Wand.identifiers)) {
      return Option(new Wand(rarity, base, name, itemLevel, identified, quality))
    }
